...
- **Reference genome** — Uncompressed FASTA (`.fa`) or gzip-compressed (`.fa.gz`)
...
```bash
docker run --rm -v $(pwd):/ftmp dnalinux/bcftools index /ftmp/snps_filtered.vcf.gz
docker run --rm -v $(pwd):/ftmp dnalinux/bcftools index /ftmp/indels_filtered.vcf.gz
```
...
```bash
docker run --rm -v $(pwd):/ftmp dnalinux/bcftools concat -a --rm-dups all \
...
```bash
docker run --rm -v $(pwd):/ftmp -w /ftmp dnalinux/bcftools index combined_filtered.vcf.gz
```
...
List the samples present in the VCF to confirm the exact identifiers to use in Step 4:

```bash
docker run --rm -v $(pwd):/ftmp -w /ftmp dnalinux/bcftools query -l combined_filtered.vcf.gz
```

### Step 4: Prepare the Reference Genome

The reference must be uncompressed before `bcftools consensus` can use it:
...
If the reference is already uncompressed, skip this step.
...
```bash
docker run --rm -v $(pwd):/ftmp -w /ftmp dnalinux/bcftools consensus \
  -f reference.fa \
  -s "SAMPLE_NAME" \
  -o SAMPLE_NAME_assembly.fasta \
  combined_filtered.vcf.gz
```
...
Key flags:
- `-f`: Path to the uncompressed reference FASTA.
